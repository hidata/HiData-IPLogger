...
- ذخیره در جدول اختصاصی `mod_iplogger` با ستون‌های: `id`, `client_id`, `action`, `ip`, `asn`, `country`, `agent`, `time`.
...
- پاک‌سازی خودکار بر اساس مدت نگهداری و غنی‌سازی کشور/ASN با کرون ساعتی.
...
- رکوردهای بدون کشور/ASN را با استفاده از `ip-api.com` تکمیل می‌کند (حداکثر 20 رکورد در هر اجرا).
